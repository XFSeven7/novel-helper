...
export function bookSummaryFromMeta(
  meta: BookMeta,
  chapterCount: number,
  missingChapterIndexes: number[] = []
): BookSummary {
...
async function missingChapterIndexesFromDir(bookDir: string): Promise<number[]> {
  const chaptersDir = path.join(bookDir, "chapters");
...
export async function listBooks(dataDir: string): Promise<BookSummary[]> {
  ...
  for (const slug of dirs) {
    ...
    try {
      ...
      metas.push(bookSummaryFromMeta(meta, chapterCount, missingChapterIndexes));
    } catch {
      // ignore broken meta
    }
  }
...
export async function createBook(dataDir: string, slug: string, title: string, synopsis?: string) {
  await ensureDir(dataDir);
  const bookDir = path.join(dataDir, slug);
  ...
  const meta: BookMeta = { slug, title, createdAt: new Date().toISOString(), synopsis: syn };
