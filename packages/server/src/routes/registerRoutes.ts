...
export async function registerRoutes(app: any, input: { dataDir: string }) {
  const { dataDir } = input;
  ...
  function buildAuditPrompt(input: { chapterTitle: string; chapterFilename: string; content: string; knownCharacters: string[] }) {
  ...
  async function finalizeAuditFromJsonText(slug: string, filename: string, jsonText: string) {
    // NOTE: 这里的逻辑保持原样，确保“增量合并/locks/relations.types/发生过的事情”等行为不变
    ...
    const uniqStrs = (arr: any) => [...new Set((Array.isArray(arr) ? arr : []).map((x) => String(x).trim()).filter(Boolean))];
    ...
    idx.characters = [...byName.values()].sort((a: any, b: any) => String(a?.name || "").localeCompare(String(b?.name || ""), "zh-Hans-CN"));
    ...
    for (const ev of run?.entities?.events || []) {
      if (!ev || typeof ev !== "object") continue;
      const cand = (ev as any).place ?? (ev as any).location ?? (ev as any).where ?? (ev as any)["地点"] ?? (ev as any)["发生地点"];
      const name = String(cand || "").trim();
      if (!name) continue;
      const note = String((ev as any).summary || (ev as any).what || (ev as any).event || "").trim() || String(run.gistL1 || "").trim();
    ...
    for (const ev of run?.entities?.events || []) {
      if (!ev || typeof ev !== "object") continue;
      const cand = (ev as any).org ?? (ev as any).organization ?? (ev as any).faction ?? (ev as any)["组织"] ?? (ev as any)["势力"];
      const name = String(cand || "").trim();
      if (!name) continue;
      const note = String((ev as any).summary || (ev as any).what || (ev as any).event || "").trim() || String(run.gistL1 || "").trim();
    ...
    orgsIdx.orgs = [...orgExisting.values()].sort((a: any, b: any) => String(a.name).localeCompare(String(b.name), "zh-Hans-CN"));
    ...
    const normTitle = (x: any) =>
      String(x?.title || x?.item || x?.name || x?.description || x?.question || x?.hook || x?.setup || x?.payoff || "").trim();
    const normProgress = (x: any) =>
      String(x?.progress || x?.update || x?.推进 || x?.note || x?.why || x?.summary || x?.expectedResolution || x?.resolution || "").trim();
  ...
  async function performPolishWithAiSdk(input: { slug: string; filename: string; modelConfigId: string | null | undefined; original: string; onDelta?: (textDelta: string) => void }) {
  ...
  app.get("/api/settings/model-configs", async () => await readModelSettings());

  app.put("/api/settings/model-configs", async (req: any) => {
  ...
  app.post("/api/books", async (req: any, reply: any) => {
    const bodySchema = z.object({
  ...
  app.patch("/api/books/:slug", async (req: any, reply: any) => {
  ...
  app.delete("/api/books/:slug", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
    try {
  ...
  app.post("/api/books/:slug/restore", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
    try {
  ...
  app.get("/api/books/:slug/chapters", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/chapters", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      title: z.string().min(1),
  ...
  app.get("/api/books/:slug/chapters/:filename", async (req: any, reply: any) => {
  ...
  app.put("/api/books/:slug/chapters/:filename", async (req: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.patch("/api/books/:slug/chapters/:filename", async (req: any, reply: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.delete("/api/books/:slug/chapters/:filename", async (req: any, reply: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.get("/api/books/:slug/story", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/story/characters", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      name: z.string().min(1),
  ...
  app.get("/api/books/:slug/story/file", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.put("/api/books/:slug/story/file", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.post("/api/books/:slug/story/characters/merge", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
  ...
  app.post("/api/books/:slug/chapters/:filename/audit", async (req: any, reply: any) => {
  ...
  app.post("/api/books/:slug/chapters/:filename/audit/stream", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1), filename: z.string().min(1) });
    const bodySchema = z.object({ modelConfigId: z.string().nullable().optional() });
    const params = paramsSchema.parse(req.params);
    const body = bodySchema.parse(req.body);
  ...
  app.post("/api/books/:slug/chapters/:filename/polish/stream", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1), filename: z.string().min(1) });
    const bodySchema = z.object({
      modelConfigId: z.string().nullable().optional(),
  ...
  app.post("/api/books/:slug/chapters/:filename/expand/stream", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1), filename: z.string().min(1) });
    const bodySchema = z.object({
      modelConfigId: z.string().nullable().optional(),
      original: z.string().optional(),
  ...
  app.get("/api/books/:slug/audit/latest", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.get("/api/books/:slug/audit/ledger", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.get("/api/books/:slug/audit/characters", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/audit/characters/hide", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.post("/api/books/:slug/audit/characters/update", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      name: z.string().min(1),
    ...
    idx.characters[i] = {
      ...
      socialTags:
        body.socialTags !== undefined
          ? {
              ...
              ...(Array.isArray((body as any).socialTags?.titles) ? { titles: mergeStrArr((prev.socialTags as any)?.titles, (body as any).socialTags?.titles) } : null),
              ...(Array.isArray((body as any).socialTags?.other) ? { other: mergeStrArr((prev.socialTags as any)?.other, (body as any).socialTags?.other) } : null)
            }
          : prev.socialTags,
      ...
      narrativeDrives:
        body.narrativeDrives !== undefined
          ? {
              ...
              ...(hasVal((body as any).narrativeDrives?.moralCompass) ? { moralCompass: normStr((body as any).narrativeDrives?.moralCompass) } : null),
              ...(Array.isArray((body as any).narrativeDrives?.flaws) ? { flaws: mergeStrArr((prev.narrativeDrives as any)?.flaws, (body as any).narrativeDrives?.flaws) } : null),
              ...(Array.isArray((body as any).narrativeDrives?.blindSpots) ? { blindSpots: mergeStrArr((prev.narrativeDrives as any)?.blindSpots, (body as any).narrativeDrives?.blindSpots) } : null)
            }
      ...
      fingerprints:
        body.fingerprints !== undefined
          ? {
              ...
              ...(Array.isArray((body as any).fingerprints?.linguisticStyle) ? { linguisticStyle: mergeStrArr((prev.fingerprints as any)?.linguisticStyle, (body as any).fingerprints?.linguisticStyle) } : null),
              ...(Array.isArray((body as any).fingerprints?.catchphrases) ? { catchphrases: mergeStrArr((prev.fingerprints as any)?.catchphrases, (body as any).fingerprints?.catchphrases) } : null),
              ...(Array.isArray((body as any).fingerprints?.mannerisms) ? { mannerisms: mergeStrArr((prev.fingerprints as any)?.mannerisms, (body as any).fingerprints?.mannerisms) } : null),
              ...(Array.isArray((body as any).fingerprints?.mask) ? { mask: mergeMask((prev.fingerprints as any)?.mask, (body as any).fingerprints?.mask) } : null)
            }
          : prev.fingerprints,
      relationalHooks:
        body.relationalHooks !== undefined
          ? {
              ...
              ...(Array.isArray((body as any).relationalHooks?.relations) ? { relations: mergeRelations((prev.relationalHooks as any)?.relations, (body as any).relationalHooks?.relations) } : null),
              ...(hasVal((body as any).relationalHooks?.freeText) ? { freeText: mergeFreeText((prev.relationalHooks as any)?.freeText, (body as any).relationalHooks?.freeText) } : null)
            }
      ...
      personalityAnalysis: body.personalityAnalysis !== undefined ? body.personalityAnalysis : prev.personalityAnalysis,
      updatedAt: now
    };
    idx.updatedAt = now;
    await writeAuditCharactersIndex(dataDir, params.slug, idx);
  ...
  app.post("/api/books/:slug/audit/characters/merge", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
    ...
    const mergeOne = (input: { idx: any; primaryName: string; secondaryName: string }) => {
      ...
      const merged = {
        ...
        fingerprints: (() => {
          ...
          if (primary?.fingerprints || secondary?.fingerprints) {
  ...
  app.post("/api/books/:slug/audit/characters/merge/preview", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      primaryName: z.string().min(1),
      secondaryNames: z.array(z.string().min(1)).min(1),
    ...
    try {
      ...
      const prompt = [
        ...
        JSON.stringify(secondaryProfiles || [], null, 2),
        "",
        "现在输出 JSON："
      ].join("\n");
  ...
  app.post("/api/books/:slug/audit/characters/merge/apply", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      primaryName: z.string().min(1),
      secondaryNames: z.array(z.string().min(1)).min(1),
    ...
    idx.characters = nextChars.sort((a: any, b: any) =>
      String(a?.name || "").localeCompare(String(b?.name || ""), "zh-Hans-CN")
    );
    idx.updatedAt = now;
    await writeAuditCharactersIndex(dataDir, params.slug, idx);
  ...
  app.get("/api/books/:slug/audit/places", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/audit/places/hide", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({ name: z.string().min(1), hidden: z.boolean() });
    const params = paramsSchema.parse(req.params);
    const body = bodySchema.parse(req.body);
  ...
  app.post("/api/books/:slug/audit/places/update", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      name: z.string().min(1),
  ...
  app.post("/api/books/:slug/audit/places/merge/preview", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      primaryName: z.string().min(1),
      secondaryNames: z.array(z.string().min(1)).min(1),
  ...
  app.post("/api/books/:slug/audit/places/merge/apply", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      primaryName: z.string().min(1),
      secondaryNames: z.array(z.string().min(1)).min(1),
  ...
  app.get("/api/books/:slug/audit/orgs", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/audit/orgs/hide", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({ name: z.string().min(1), hidden: z.boolean() });
    const params = paramsSchema.parse(req.params);
    const body = bodySchema.parse(req.body);
  ...
  app.post("/api/books/:slug/audit/orgs/update", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      name: z.string().min(1),
      description: z.string().optional(),
  ...
  app.get("/api/books/:slug/audit/foreshadows", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/audit/foreshadows/create", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      title: z.string().min(1),
  ...
  app.post("/api/books/:slug/audit/foreshadows/update", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      id: z.string().min(1),
    ...
    idx.foreshadows[i] = {
      ...
      lastChapter: chapters !== undefined ? (chapters.length ? chapters[chapters.length - 1] : undefined) : prev.lastChapter,
      updatedAt: now
    };
    idx.updatedAt = now;
    await writeAuditForeshadowsIndex(dataDir, params.slug, idx);
  ...
  app.post("/api/books/:slug/audit/foreshadows/hide", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.get("/api/books/:slug/timeline/index", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/timeline/event/mark", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  app.post("/api/books/:slug/timeline/compress", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
  ...
  app.post("/api/books/:slug/timeline/range/delete", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
  ...
  // -----------------------------
  // 灵感库（inspiration.json）
  // -----------------------------

  app.get("/api/books/:slug/inspiration", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
  ...
  app.post("/api/books/:slug/inspiration/upsert", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      item: z.object({
    ...
    if (!incoming) return { ok: false, message: "Invalid item" };
  ...
  app.post("/api/books/:slug/inspiration/status", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      id: z.string().min(1),
  ...
  app.post("/api/books/:slug/inspiration/purge", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
    const idx = normalizeInspirationIndex(await readInspirationIndex(dataDir, params.slug));
    const before = idx.items.length;
    idx.items = idx.items.filter((x) => x.status !== "deleted");
    if (idx.items.length === before) return { ok: true, index: idx, purged: 0 };
    idx.updatedAt = new Date().toISOString();
    await writeInspirationIndex(dataDir, params.slug, idx);
    return { ok: true, index: idx, purged: before - idx.items.length };
  });

  app.post("/api/books/:slug/inspiration/generate", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      modelConfigId: z.string().nullable().optional(),
    ...
    const schemaHint = [
      ...
      "content 必须是可直接用于写作/设定的中文文本。"
    ].join("\n");

    const prompt = [
      "你是网络小说写作助手，负责生成可直接落地写作的“灵感卡片”。",
      schemaHint,
      "",
      `count = ${count}`,
      `生成类型 kind = ${kind}`,
      opts && Object.keys(opts).length ? `可选项 options = ${JSON.stringify(opts, null, 2)}` : "可选项 options = {}",
      free ? `自由输入 freeText = ${free}` : "自由输入 freeText = （空）",
      "",
      useMemory ? "【参考全书记忆】（用于一致性与避重复）\n" + memoryText : "【参考全书记忆】（未启用）",
      "",
      "现在输出 JSON 数组："
    ].join("\n");
    ...
    const arr = safeJsonParse<any[]>(String(text || "")) || [];
    ...
    idx.updatedAt = now;
    await writeInspirationIndex(dataDir, params.slug, idx);
    return { ok: true, index: idx, items };
  });

  app.post("/api/books/:slug/inspiration/variant", async (req: any, reply: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      modelConfigId: z.string().nullable().optional(),
      id: z.string().min(1),
  ...
  app.post("/api/novels", async (req: any, reply: any) => {
    const bodySchema = z.object({
      title: z.string().min(1),
      slug: z.string().optional(),
      synopsis: z.string().max(20000).optional()
  ...
  app.get("/api/novels/:slug/chapters", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const params = paramsSchema.parse(req.params);
    const chapters = await listChapters(dataDir, params.slug);
    return { chapters };
  ...
  app.post("/api/novels/:slug/chapters", async (req: any) => {
    const paramsSchema = z.object({ slug: z.string().min(1) });
    const bodySchema = z.object({
      title: z.string().min(1),
      content: z.string().optional(),
  ...
  app.get("/api/novels/:slug/chapters/:filename", async (req: any, reply: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.put("/api/novels/:slug/chapters/:filename", async (req: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.patch("/api/novels/:slug/chapters/:filename", async (req: any, reply: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
  ...
  app.delete("/api/novels/:slug/chapters/:filename", async (req: any, reply: any) => {
    const paramsSchema = z.object({
      slug: z.string().min(1),
      filename: z.string().min(1)
    });
